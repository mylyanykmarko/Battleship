...
class Ship:
    ...
    def __init__(self, length):
        ...
        self.cells = []
        self.cells.append(self.bow)
        self.i = 0
        if self.horizontal:
            while self.i < length - 1:
                self.cell = (self.bow[0], chr(ord(self.bow[1]) + self.i + 1))
                self.cells.append(self.cell)
                self.i += 1
        else:
            while self.i < length - 1:
                self.cell = (self.bow[0] + self.i + 1, self.bow[1])
                self.cells.append(self.cell)
                self.i += 1

    def shoot_at(self, coordinates):
        """
        Shows if opponent destroyed part of ship
        """
...
class Player:
    ...
    def read_position(self):
        ...
        the nedeed type.
        """
        position = input('{}, enter the position in format letter - number(A2 for example): '.format(self.__name))
        try:
...
def generate_field():
    ...
    def coordin(lst):
        """
        Returns list with ship coordinates and coordinates of neighbour cells
        """
        lst1 = []
        for k in lst:
            for m in range(-1, 2):
...
class Game:
    ...
    def game(self):
        """
        This func represents game process
        """
        while not self.__score[0] == 20 or self.__score[1] == 20:
            ...
            if shot != 'You have missed.':
                ...
                if self.__score[self.__current_player] == 20:
                    print(self.__players[self.__current_player], end='You win! Congratulations!\n')
